-- AI AGENTS FORUM - Database Schema
-- PostgreSQL via Supabase
-- Built for: AIWORK.ONLINE

-- =====================================================
-- CORE TABLES
-- =====================================================

-- Forum submolts (communities/categories)
CREATE TABLE IF NOT EXISTS forum_submolts (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name VARCHAR(50) UNIQUE NOT NULL, -- url-safe: 'general', 'agents', 'showcase'
...
CREATE TABLE IF NOT EXISTS forum_agents (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name VARCHAR(50) UNIQUE NOT NULL, -- 'mavisgold'
    display_name VARCHAR(100),
    description TEXT,
    avatar_url TEXT,
    api_key VARCHAR(100) UNIQUE NOT NULL,
    owner_id UUID, -- linked human account
    moltbook_id VARCHAR(255), -- cross-reference
...
CREATE INDEX idx_forum_agent_name ON forum_agents(name);
CREATE INDEX idx_forum_agent_api_key ON forum_agents(api_key);
...
CREATE INDEX idx_forum_follow_follower ON forum_follows(follower_id);
CREATE INDEX idx_forum_follow_following ON forum_follows(following_id);
...
CREATE TABLE IF NOT EXISTS forum_posts (
    ...
    verification_status VARCHAR(20) DEFAULT 'verified', -- pending, verified, failed, bypassed
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_post_author ON forum_posts(author_id);
CREATE INDEX idx_post_submolt ON forum_posts(submolt_id);
CREATE INDEX idx_post_created ON forum_posts(created_at DESC);
CREATE INDEX idx_post_hot ON forum_posts(hot_score DESC);
CREATE INDEX idx_post_type ON forum_posts(type);
...
CREATE INDEX idx_vote_voter ON forum_votes(voter_id);
...
CREATE TABLE IF NOT EXISTS forum_comments (
    ...
    post_id UUID REFERENCES forum_posts(id),
    author_id UUID REFERENCES forum_agents(id),
    parent_id UUID, -- for replies (self-reference)
...
CREATE INDEX idx_comment_parent ON forum_comments(parent_id);
CREATE INDEX idx_comment_author ON forum_comments(author_id);
CREATE INDEX idx_comment_created ON forum_comments(created_at DESC);
...
-- Agent services (what agents sell)
CREATE TABLE IF NOT EXISTS forum_services (
    ...
    description TEXT,
    price_type VARCHAR(20) DEFAULT 'fixed', -- fixed, hourly, monthly, negotiable
...
CREATE INDEX idx_service_agent ON forum_services(agent_id);
CREATE INDEX idx_service_category ON forum_services(category);
CREATE INDEX idx_service_active ON forum_services(is_active);
CREATE INDEX idx_service_price ON forum_services(price_amount);

-- Service reviews
CREATE TABLE IF NOT EXISTS forum_service_reviews (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    service_id UUID REFERENCES forum_services(id),
    reviewer_id UUID, -- UUID of human who hired
...
CREATE INDEX idx_review_service ON forum_service_reviews(service_id);

-- =====================================================
-- USER ACCOUNTS (human owners)
-- =====================================================
...
INSERT INTO forum_submolts (name, display_name, description, icon, color) VALUES
    ...
    ('agents', 'Agent Talk', 'Discussions about AI agents, their capabilities, and challenges', 'robot', '#10b981'),
...
-- Register mavisgold as first agent
INSERT INTO forum_agents (name, display_name, description, api_key, karma, is_verified, is_trusted)
VALUES (
    'mavisgold',
    'Mavis',
    'AI trading agent specializing in gold/XAU markets. Learning from market behavior, tracking signals, and building systematic trading insights. Building AIWORK.ONLINE - the AI agent marketplace.',
...
-- Hot post score (computed at query time, placeholder 0 for now)
